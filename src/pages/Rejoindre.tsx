    ...
    try {
      ...
      if (hasGuarantee) {
        const result = await initPayment({
          ...
          operator: "MTN", // Valeur par défaut pour l'interface simplifiée
        });
        if (!result.success) {
      ...
      <div className="flex flex-col min-h-screen animate-fade-in">
        <div className="flex-1 px-4 pt-12 text-center">
          ...
          <p className="text-xs text-muted-foreground mb-8">
            {hasGuarantee ? "Votre caution a été payée et vous êtes maintenant membre." : "Vous êtes maintenant membre du groupe."}
          </p>
    ...
    <div className="animate-slide-up">
      ...
      <div className="px-4 pb-6">
        ...
        {hasGuarantee && (
          <>
            <PhoneInput
              label="Numéro pour le paiement de la caution"
              value={phone}
              onChange={setPhone}
              className="mb-4"
            />
            <div className="p-3 rounded-xl bg-[hsla(38,92%,50%,0.08)] border border-[hsla(38,92%,50%,0.15)] mb-4">
              ...
              <p className="text-xs text-muted-foreground">
                <strong className="text-foreground">{formatFCFA(group.guarantee_deposit)}</strong> — payé via TalyPay et remboursé en fin de cycle
              </p>
            </div>
          </>
        )}
        ...
        {hasGuarantee && !alreadyMember && (
          <p className="text-[10px] text-muted-foreground text-center mt-2">🔒 Paiement sécurisé via TalyPay</p>
        )}
      </div>
    </div>
  );
